import {
  getCoursesWithResourceByName,
} from "@/lib/actions/courses/getCourses";
...
export default async function CourseIntroSection({
  name,
  type,
}: CourseTemplateProps) {
  const courses: CourseWithResourceInterface[] =
    await getCoursesWithResourceByName(name);
  ...
  const resourse =
    courses.length > 0 ? courses[0].resource : { name: "", description: "" };
    ...
    <div className="flex-1 ml-8">
      <div className="mb-8 relative">
        <div
          ...
          style={{
            backgroundImage: "radial-gradient(#1f1f1f 0.2px, transparent 1px)",
